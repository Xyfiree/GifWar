...
async function startRound(code) {
  ...
  if (room.settings.promptSource === 'players') {
    // Pick random writer
    const writerIdx = Math.floor(Math.random() * room.players.length);
    const writer = room.players[writerIdx];
    room.currentWriter = writer.id;
    room.state = 'writing';

    // Tell writer to write
    io.to(writer.id).emit('writer-turn', {
    ...
    room.players.forEach(p => {
      if (p.id !== writer.id) {
        io.to(p.id).emit('waiting-for-writer', {
          round: room.currentRound,
          totalRounds: room.settings.rounds,
...
io.on('connection', (socket) => {
  ...
  socket.on('create-room', ({ username, mode, rounds, timer, promptSource }) => {
    if (!username?.trim()) return socket.emit('error', 'Pseudo requis.');
    const code = generateCode();
    const room = {
      code,
      host: socket.id,
      players: [{ id: socket.id, username: username.trim(), score: 0, egk: 0 }],
      settings: {
        mode: parseInt(mode) || 1,
        rounds: parseInt(rounds) || 5,
        timer: timer ? 60 : null,
        promptSource: promptSource || 'ai'
      },
      state: 'lobby',
      currentRound: 0,
      currentPrompt: null,
  ...
  socket.on('join-room', ({ code, username }) => {
    ...
    room.players.push({ id: socket.id, username: uname, score: 0, egk: 0 });
  ...
  socket.on('update-settings', ({ mode, rounds, timer, promptSource }) => {
  ...
  socket.on('submit-prompt', ({ prompt }) => {
    const code = socket.data.code;
    const room = rooms.get(code);
    if (!room || room.state !== 'writing') return;
    if (socket.id !== room.currentWriter) return;
    const trimmed = prompt?.trim();
    if (!trimmed || trimmed.length < 3) return socket.emit('error', 'La phrase est trop courte !');

    room.currentPrompt = trimmed;
    room.state = 'playing';
    room.currentWriter = null;
    ...
    if (room.settings.timer) {
      const t = setTimeout(() => {
        const r = rooms.get(code);
        if (r && r.state === 'playing' && r.currentRound === room.currentRound) startVoting(code);
      }, room.settings.timer * 1000);
      timers.set(code + '_submit', t);
    }
  });

  socket.on('award-egk', ({ targetId }) => {
    const code = socket.data.code;
    const room = rooms.get(code);
    if (!room || room.state !== 'voting') return;
    if (room.egkUsed[socket.id]) return; // already used EGK this round
    if (targetId === socket.id) return; // can't give to yourself
    if (!room.submissions[targetId]) return; // invalid target

    room.egkUsed[socket.id] = targetId;
    const target = room.players.find(p => p.id === targetId);
    if (target) target.egk = (target.egk || 0) + 1;

    socket.emit('egk-confirmed', { targetId });
    io.to(code).emit('egk-awarded', { fromId: socket.id, targetId, targetName: target?.username });
  });
  ...
  socket.on('play-again', () => {
    ...
    room.egkUsed = {};
    room.currentWriter = null;
    room.players.forEach(p => { p.score = 0; p.egk = 0; });
