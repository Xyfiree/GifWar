...
async function generateConversation() {
  try {
    const text = await callGemini(
      `Tu crées des prompts pour un jeu de GIFs multijoueur.
Génère une courte conversation drôle ou awkward entre 2 personnes (3 à 5 répliques max).
Les joueurs devront trouver le GIF de réaction parfait pour cette conversation.
Utilise ce format exact (une réplique par ligne) :
🧑 A : ...
🧑 B : ...
🧑 A : ...
Sois créatif, drôle, relatable, en FRANÇAIS. Pas de preamble, juste la conversation.`
    );
    return text || "🧑 A : T'as un secret à me dire ?\n🧑 B : Oui... j'aime pas les pizzas.\n🧑 A : On peut plus être amis.";
  } catch (e) {
    return "🧑 A : T'as un secret à me dire ?\n🧑 B : Oui... j'aime pas les pizzas.\n🧑 A : On peut plus être amis.";
  }
}
...
async function generatePhrase() {
  try {
    const text = await callGemini(
      `Tu crées des prompts pour un jeu de GIFs multijoueur.
Génère UNE SEULE situation drôle ou relatable. Les joueurs devront trouver le GIF parfait.
Commence par "Quand..." ou "Toi quand..." ou une courte description de situation.
Ajoute un ou deux emojis. En FRANÇAIS. UNE seule phrase, pas de guillemets, pas de preamble.
Exemple : "Quand tu réalises que c'est lundi demain 😱"`
    );
    return text.split('\n')[0].trim() || "Quand ton alarme sonne le matin 😴";
  } catch (e) {
    return "Quand tu réalises que tu as oublié quelque chose d'important 😬";
  }
}
...
server.listen(PORT, () => console.log(`🎮 GifWar running → http://localhost:${PORT}`));
